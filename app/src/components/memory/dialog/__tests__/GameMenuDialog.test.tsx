...
describe('GameMenuDialog', () => {
  ...
  it('Should not show the dialog when closed', () => {
    render(<GameMenuDialog open={false} onClose={vi.fn()} onRestart={vi.fn()} onNewGame={vi.fn()} />);

    expect(screen.queryByText(/Restart/)).not.toBeInTheDocument();
  });

  it('Should show the dialog when open', () => {
    render(<GameMenuDialog open onClose={vi.fn()} onRestart={vi.fn()} onNewGame={vi.fn()} />);
  ...
  it('Should handle the close dialog event', () => {
    ...
    const { container } = render(<GameMenuDialog open onClose={onClose} onRestart={vi.fn()} onNewGame={vi.fn()} />);
  ...
  it('Should handle button clicks correctly', () => {
    ...
    render(<GameMenuDialog open onClose={onClose} onRestart={onRestart} onNewGame={onNewGame} />);
