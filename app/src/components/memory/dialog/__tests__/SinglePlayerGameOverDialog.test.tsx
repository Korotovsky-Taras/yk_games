...
describe('SinglePlayerGameOverDialog', () => {
  ...
  it('Should not be visible when closed', () => {
    render(<SinglePlayerGameOverDialog open={false} seconds={201} moves={26} onRestart={vi.fn()} onSetupNewGame={vi.fn()} />);

    expect(screen.queryByText(/Победа!/)).not.toBeInTheDocument();
  });

  it('Should display the dialog values correctly', () => {
    render(<SinglePlayerGameOverDialog open seconds={201} moves={26} onRestart={vi.fn()} onSetupNewGame={vi.fn()} />);
  ...
  it('Should repass the event handlers to the GameOverDialog component', () => {
    ...
    render(<SinglePlayerGameOverDialog open seconds={201} moves={26} onRestart={onRestart} onSetupNewGame={onSetupNewGame} />);
