...
import { getPlayerColor } from '~/utils/memoryGame';
...
describe('MultiPlayerGameOverDialog', () => {
  ...
  it('Should not show the dialog when closed', () => {
    render(
      <MultiPlayerGameOverDialog
        ...
        players={[
          { name: '1', points: 1, color: getPlayerColor(1) },
          { name: '2', points: 2, color: getPlayerColor(2) },
        ]}
      />,
    );
  ...
  it('Should show a single winner', () => {
    ...
    expect(screen.queryByText(/Победа игрока 2!/)).toBeInTheDocument();
  ...
  it('Should show multiple winners', () => {
    render(
      <MultiPlayerGameOverDialog
        ...
        onSetupNewGame={vi.fn()}
        players={[
          { name: '1', points: 4, color: getPlayerColor(1) },
          { name: '2', points: 0, color: getPlayerColor(2) },
          { name: '3', points: 1, color: getPlayerColor(3) },
          { name: '4', points: 4, color: getPlayerColor(4) },
        ]}
      />,
    );
