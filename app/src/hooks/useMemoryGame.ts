...
import { MemorySettings, revealWaitTime } from '~/utils/memoryConfig';
    ...
    case 'finishTurn':
      return produce(state, (draft: Draft<State>) => {
        ...
        if (revealedCard1.value === revealedCard2.value) {
          ...
          revealedCard1.color = draft.players[draft.currentPlayer].color;
          revealedCard2.color = draft.players[draft.currentPlayer].color;
          draft.players[draft.currentPlayer].points++;
          draft.pairsLeft--;
        } else {
          revealedCard1.state = 'hidden';
          revealedCard2.state = 'hidden';
          draft.currentPlayer = draft.currentPlayer === draft.players.length - 1 ? 0 : draft.currentPlayer + 1;
        }
...
export function useMemoryGame(settings: MemorySettings) {
